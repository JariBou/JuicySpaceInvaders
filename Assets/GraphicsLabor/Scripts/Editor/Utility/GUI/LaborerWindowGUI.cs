...
namespace GraphicsLabor.Scripts.Editor.Utility.GUI
{
    ...
    public static class LaborerWindowGUI
    {
        ...
        public static float DrawScriptableObjectButtons(Rect startRect, float yOffset, SerializedObject serializedObject)
        {
            float localOffset = 0;
            
            IEnumerable<MethodInfo> methods = ReflectionUtility.GetAllMethods(
                serializedObject.targetObject, m => m.GetCustomAttributes(typeof(ButtonAttribute), true).Length > 0);
            ...
            foreach (MethodInfo methodInfo in methodInfos)
            {
                ...
                LaborerEditorGUI.Button(serializedObject.targetObject, methodInfo, newRect);
                localOffset += LaborerGUIUtility.SingleLineHeight + LaborerGUIUtility.PropertyHeightSpacing;
            }
            return localOffset;
        }
